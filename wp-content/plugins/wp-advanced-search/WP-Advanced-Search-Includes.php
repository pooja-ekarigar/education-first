...
function addAutoCompletion() {
	...
	if($select->autoCompleteActive == 1) {
		...
		// Paramètres Ajax
		wp_enqueue_script('params-autocomplete', plugins_url("class.inc/autocompletion/params.js", __FILE__ ), array('autocomplete'), false, true);
		$scriptData = array(
			'selector' => $selector,
...
function WP_Advanced_Search_AutoCompletion() {
	...
	if($select->autoCompleteActive == 1) {
		...
		wp_enqueue_script('autocomplete', $url, array('jquery'), false, true);
	}
}
